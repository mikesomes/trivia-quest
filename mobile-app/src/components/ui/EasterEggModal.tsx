...
const MESSAGES: { icon: IconName; title: string; body: string }[] = [
  { icon: 'trivia', title: 'Big Brain Energy', body: "You tapped a greeting seven times.\nWe respect the curiosity." },
  { icon: 'coffee', title: 'Developer Secret', body: "This app was built on coffee,\nquestionable decisions, and vibes." },
  { icon: 'target', title: 'Pro Tip', body: "The answer is usually B.\n(Please don't actually use this strategy.)" },
  { icon: 'mythical', title: 'You Found the Unicorn', body: "Unfortunately it's just a\ntext message. Sorry." },
  { icon: 'detective', title: 'Secret Agent', body: "You have the instincts of someone\nwho reads every EULA. Rare." },
  { icon: 'idea', title: 'Fun Fact', body: "The average person blinks 15 times\na minute. You blinked at least twice\nreading this." },
]

/** The wobbling mark is decorative — the title and body carry the joke. */
const MARK_SIZE = 64
...
export function EasterEggModal({ visible, onDismiss }: Props) {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(markRotate, { toValue: 1, duration: 600, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(markRotate, { toValue: -1, duration: 600, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(markRotate, { toValue: 0, duration: 300, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.delay(800),
      ])
    ).start()
  }, [visible])

  const rotateDeg = markRotate.interpolate({
    inputRange: [-1, 0, 1],
          ...
          <TouchableWithoutFeedback>
            <Animated.View style={[styles.card, { opacity: cardOpacity, transform: [{ scale: cardScale }] }]}>
              <Animated.View style={{ transform: [{ rotate: rotateDeg }] }}>
                <AppIcon name={message.icon} size={MARK_SIZE} />
              </Animated.View>
