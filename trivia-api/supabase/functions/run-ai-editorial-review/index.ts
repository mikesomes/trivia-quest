...
Deno.serve(async req => {
  ...
  const db = createServiceClient(); const { data: candidates, error } = await db.from('question_candidates').select('id,question_text,choices,correct_answer,explanation,difficulty_rating,blind_review_answer_index,blind_review_matches,reviewer_confidence,blind_reviewer_notes,verification_status,verification_source,verification_notes').in('editorial_status', ['pending', 'revise']).is('editorial_reviewed_at', null).order('created_at').limit(limit)
